...
#01/07/2020 - because couldn't get M_start/M_end to work for some reason)
Ebal_denominator <- met$NR[2461:8174]-met$G[2461:8174]
...
write.csv(Data_QAQC,paste('./flux_data/BB_L2','.csv',sep=''),row.names=FALSE)   
```
...
output <- output[c("year", "jday", "hour_dec","co2_flux","ch4_flux","LE","H","SHORTWAVE_IN","AIR_TEMP_2M","SOIL_TEMP_1_5CM","SOIL_TEMP_1_10CM", "RH_2M","VPD_hPa","u.","WTH")]

names_output<-c('Year','DoY','Hour','NEE','FCH4','LE','H','Rg','Tair','Tsoil5cm','Tsoil10cm','rH','VPD','Ustar','WTH')
names(output)<-names_output
...
#Saving the file:
write.table(output, file = paste('./flux_data/REddyProc_input/for_gap_filling_partitioning','.txt',sep=''),row.names=FALSE,sep='\t')   
```
